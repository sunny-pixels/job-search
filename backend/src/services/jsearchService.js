 ...
 * Uses job_keywords (similar roles) for broader job search
...
const buildSearchQueries = (resumeAnalysis) => {
  // Use job_keywords (similar roles generated by LLM)
  ...
  // Use keywords as-is (similar job titles from LLM)
  ...
  if (experienceYears > 0) {
    ...
    if (searchTerms.length < keywords.length) {
      console.log(`   ⚠️ Filtered out intern positions (candidate has ${experienceYears} years experience)`);
    }
  } else {
    console.log(`   ℹ️ Including all positions (candidate has 0 years experience)`);
  }
  ...
  // Get unique terms (use original casing from LLM)
  const uniqueTerms = Array.from(normalizedMap.values());
  
  // Limit to top 3-4 search terms to avoid too many API calls
  const topTerms = uniqueTerms.slice(0, 4);
  
  console.log(`🔍 [JSearch] Built ${topTerms.length} search queries:`, topTerms);
